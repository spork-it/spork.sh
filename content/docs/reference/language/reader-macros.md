...
Reader macros are fixed prefix syntax recognized while Spork source is read into forms. They run before ordinary compilation and are distinct from user-defined macros created with `defmacro`. This page defines every reader-macro prefix in Spork 0.6.0.
